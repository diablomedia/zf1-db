...
class Zend_Db_Adapter_Pdo_MysqlTest extends Zend_Db_Adapter_Pdo_TestCommon
{
    ...
    public function testAdapterDriverOptions()
    {
        ...
        $this->assertTrue((bool) $db->getConnection()->getAttribute($bufferedQueryConst));
        $params['driver_options'] = array($bufferedQueryConst => false);

        $db = Zend_Db::factory($this->getDriver(), $params);

        $this->assertFalse((bool) $db->getConnection()->getAttribute($bufferedQueryConst));
    }
